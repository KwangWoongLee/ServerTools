...
const AlertModal = () => {
  ...
  useEffect(() => {
    if (box === null || box.current === null) return;

    if (box.current.scrollHeight > 200) box.current.style.height = '200px';
    else box.current.style.height = box.current.scrollHeight + 'px';
  }, [state]);
