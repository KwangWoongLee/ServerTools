  ...
  init(conf) {
    return new Promise((resolve) => {
      ...
      this.pub.on('error', (err) => {
        console.error(this.name, ': redis close ', err);
      });

      this.pub.on('end', (err) => {
        console.error(this.name, ': redis end ', err);
      });

      this.pub.on('connect', () => {
        console.info(`${this.name} : redis-connect-database = ${conf.db ? conf.db : 0} : ${conf.host}:${conf.port}`);
        if (this.pub.isInit == undefined) {
          ...
          setInterval(() => {
            this.pub.ping((r) => console.info(this.name, ': redis pong'));
          }, 1000 * 60 * 10);
      ...
      if (conf.subscribe) {
        ...
        this.sub.on('connect', () => {
          console.info('connect sub redis : ', sub_channel);
        });
...
const get_sio_redis_adapter = (host, port, key) => {
  ...
  pub.on('error', (err) => console.error('socket.io redis pub error : ' + err.stack));
  sub.on('error', (err) => console.error('socket.io redis sub error : ' + err.stack));
  pub.on('end', (err) => console.error('socket.io redis pub end : ' + err));
  sub.on('end', (err) => console.error('socket.io redis sub end : ' + err));
